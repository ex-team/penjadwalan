...
class M_Pengampu extends CI_Model{
	...
	function delete_by_mp($kode_mp){
		$this->db->query("DELETE FROM pengampu WHERE kode_mp = '$kode_mp'");
	}
	...
	function cek_for_update($kode,$kode_mp,$kode_guru,$kelas,$tahun_akademik){		
		$rs = $this->db->query(
							   "SELECT CAST(COUNT(*) AS CHAR(1)) as cnt".
                               "FROM pengampu ".
							   "WHERE kode_mp='$kode_mp' AND ".
	...
	function cek_for_insert($kode_mp,$kode_guru,$kelas,$tahun_akademik,$kode){		
		$rs = $this->db->query(
							   "SELECT CAST(COUNT(*) AS CHAR(1)) as cnt".
                               "FROM pengampu ".
							   "WHERE kode_mp='$kode_mp' AND ".
