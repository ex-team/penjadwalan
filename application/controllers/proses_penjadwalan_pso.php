...
class Proses_penjadwalan extends CI_Controller
{
    ...
    */
    public function build_kelas_per_makul($param)
    {
    ...
    public function buildGroupJadwalKemipaan($jumlah_kelas)
    {
    ...
    public function klasifikasi($param)
    {
        $kelas_bagi = ceil($param['jml_porsi'] / $param['batas_jml_kelas']);
        $arr_id_group_jadwal_kemipaan = [];
    ...
    public function lakukan_penjadwalan()
    {
        $this->load->library('alpso');
        $this->load->model('Penjadwalan_model');
        $this->load->library('bantu');
        ...
        if (!empty($kelas) && !empty($ruang) && !empty($waktu)) {
            ...
            /*
            ** PROSES ALGORITMA PSO
            */
            $waktu_start = microtime(true);
            $this->alpso->initialize($kelas, $ruang, $waktu, $_POST, $prodi, $min_prosen_capacity);
            for ($i = 0; $i < $_POST['iteration']; $i++) {
                if ($i == 0) {
                    $this->alpso->generate_population();
                } else {
                    $this->alpso->update_population();
                }

                $this->alpso->count_fitness();
                // $this->algoritma_pso->roulette_wheel_selection();
                // $this->algoritma_pso->crossover();
                // $this->algoritma_pso->mutation();
                // $this->algoritma_pso->update_selection();
            }
            $solusi = $this->alpso->get_solution();
            $solusi = $solusi['arr_gen'];
            $total_waktu = microtime(true) - $_SERVER['REQUEST_TIME_FLOAT'];

            $result = $this->bantu->getDataLogproses('alpso_penjadwalan');
        ...
        if ($this->db->trans_status() === true) {
            $notif = [
                'style' => 'success',
                'msg' => '<strong>Congratulation</strong> Jadwal kuliah berhasil disimpan. Waktu pemrosesan algoritma pso <strong>'.$total_waktu.'</strong> detik.',
            ];
        } else {
            $notif = [
                'style' => 'error',
